...
```bash
curl -L https://github.com/arianlopezc/Trabuco/releases/latest/download/trabuco-plugin-v1.10.1.tar.gz \
  | tar -xz -C ~
```
...
```text
/plugin marketplace add ~/trabuco-plugin-v1.10.1
/plugin install trabuco@trabuco-marketplace
```
...
| `trabuco` CLI | ≥ 1.8.0 (latest is 1.10.1) |
