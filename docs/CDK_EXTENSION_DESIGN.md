...
Existing Spring Boot projects generated by Trabuco cannot be automatically converted to CDK projects. Trabuco's migration feature (being redesigned for 1.10.0; see `docs/MIGRATION_REDESIGN_PLAN.md`) targets Spring Boot transformations only and is not extended for CDK. Instead, users start fresh with `trabuco init --target=cdk` and manually move their domain objects (Model) and business logic (Shared) into the new project. The Model and Shared module code is structurally identical between Spring Boot and CDK projects by design.
