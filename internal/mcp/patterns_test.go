...
func TestE2E_MixedUnsupported(t *testing.T) {
	// "REST API with login form, React frontend, and WebSocket chat" —
	// login form (IdP-side) + frontend + websocket are all unsupported.
	// JWT validation is intentionally omitted from this query: it IS
	// supported (auth scaffolding ships dormant with API/AIAgent), so
	// it does not belong in not_covered.
	advisory := buildAdvisory("REST API with login form, React frontend, and WebSocket chat")
	if len(advisory.NotCovered) < 3 {
		t.Errorf("Expected at least 3 not_covered items (login form, frontend, websocket), got %d", len(advisory.NotCovered))
	}
...
// TestBuildAdvisory_JwtAuthIsSupported verifies that mentioning JWT/OIDC
// auth in requirements does NOT mark auth as unsupported. The scaffolding
// is generated by default — only the IdP-side (login forms, token
// issuance, user management) is excluded.
func TestBuildAdvisory_JwtAuthIsSupported(t *testing.T) {
	advisory := buildAdvisory("REST API protected by JWT bearer tokens with OIDC")
	for _, nc := range advisory.NotCovered {
		// "Authentication identity-provider side" is the legitimate
		// IdP-side warning and should not fire on this query (no login
		// form / signup / token issuance mentioned).
		if strings.Contains(nc, "Authentication identity-provider side") {
			t.Errorf("JWT/OIDC validation IS supported; should not warn about identity-provider side. Got: %q", nc)
		}
	}
}
