...
func TestSkills_ModuleGate_AIAgentOnly(t *testing.T) {
	...
	for _, name := range []string{"add-tool", "add-guardrail-rule", "add-knowledge-entry", "add-retriever", "add-streaming-endpoint", "add-a2a-skill"} {
...
func TestSkills_ReviewOff_EmitsNothing(t *testing.T) {
	...
	reviewSkills := []string{
		...
		"add-retriever", "add-streaming-endpoint", "add-guardrail-rule", "add-a2a-skill", "pr", "commit",
	}
